  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
      <div>
        ...
        {featured && (
          <div className="mb-8 overflow-hidden rounded-lg border border-[#1F2E45]/15 bg-[#FAFAFA]">
            {featured.image ? (
              <img src={featured.image} alt={featured.name} className="h-[24rem] w-full object-cover sm:h-[28rem]" />
            ) : (
              <div className="flex h-[20rem] w-full items-center justify-center bg-[#1F2E45]/5 text-sm text-[#1F2E45]/30">
                No photo yet
              </div>
            )}
            <div className="p-6">
              <div className="flex items-start justify-between">
                <p
                  className="text-xs uppercase tracking-[0.2em] text-[#1F2E45]/40"
                  style={{ fontFamily: "'JetBrains Mono', monospace" }}
                >
                  Featured
                </p>
                <span
                  ...
                  style={{ background: KIND_TONE[featured.kind] }}
                >
                  {featured.kind}
                </span>
              </div>
              <h3 className="mt-1 text-2xl text-[#1F2E45]" style={{ fontFamily: "'Fraunces', Georgia, serif" }}>
                {featured.name}
              </h3>
              {featured.description && (
                <p className="mt-2 text-sm leading-relaxed text-[#1F2E45]/60">{featured.description}</p>
              )}
              <div className="mt-4 flex items-center justify-between">
                <p className="text-2xl text-[#1F2E45]" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                  ${featured.price}
                </p>
                <button
                  onClick={() => addToCart(featured)}
                  className="rounded-md bg-[#1F2E45] px-6 py-2 text-sm text-[#F5F0E6] transition-opacity hover:opacity-90"
                >
                  Add to cart
                </button>
              </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {rest.map((p) => (
            ...
            >
              <div>
                {p.image ? (
                  <img src={p.image} alt={p.name} className="mb-3 h-[28rem] w-full rounded-md object-cover" />
                ) : (
                  <div className="mb-3 flex h-[28rem] w-full items-center justify-center rounded-md bg-[#1F2E45]/5 text-xs text-[#1F2E45]/30">
                    No photo yet
                  </div>
                )}
                <div className="flex items-start justify-between">
